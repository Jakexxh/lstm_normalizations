...
class HIDCNLSTMCell(RNNCell):
	def __init__(self,
		...
		super(HIDCNLSTMCell, self).__init__(_reuse=reuse)
	...
	def call(self, inputs, state):
		...
		inputs = self.cosine_norm(inputs, hidden_weight, 'hidden_inputs')

		concat = tf.tanh(self._line_sep([inputs, h], 4 * self._num_units, bias=True))
	...
	def _line_sep(self,
		...
		with vs.variable_scope(scope) as outer_scope:
			...
			cn_xh = self.cosine_norm(x, W_xh, 'scale_cn_xh')
			cn_hh = self.cosine_norm(h, W_hh, 'scale_cn_hh')
			res = cn_xh + cn_hh
