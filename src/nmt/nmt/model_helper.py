...
from normal_cells.lstm_hid_cn import HIDCNLSTMCell
...
def _single_cell(unit_type, num_units, grain, step_length, forget_bias, dropout,
	...
	if unit_type == "base":
		...
		utils.print_out("  bn_sep LSTM, forget_bias=%g" % forget_bias, new_line=False)
		single_cell = BNLSTMCell(
			num_units,
			max_bn_steps=step_length,
			initial_scale=grain,
			forget_bias=forget_bias)
	elif unit_type == "gru":
...
def _cell_list(unit_type, num_units, grain, step_length, num_layers, num_residual_layers,
...
def create_rnn_cell(unit_type, num_units, grain, step_length, num_layers, num_residual_layers,
                    forget_bias, dropout, mode, num_gpus, base_gpu=0,
                    single_cell_fn=None):
