...
from normal_cells.lstm_cn_norm_sep import CNNROMLSTMCell
...
flags.DEFINE_integer("num_gpus", 0,
                     ...
                     "running one replica.")
flags.DEFINE_string("rnn_mode", 'bn_sep', "lstm cells")
		...
		for step_num in range(model.input.epoch_size):
			...
			if verbose and step_num % (model.input.epoch_size // 10) == 10:
				print(
					...
					(step_num * 1.0 / model.input.epoch_size,
					 np.exp(costs / iters),
		...
		with sv.managed_session(config=config_proto) as session:
			...
			with open(save_path + "/log.txt", "w+") as myfile:
				...
				myfile.write("final Train Perplexity: " + str(train_perplexity)
				             + '\n')
				myfile.write("final Valid Perplexity: " + str(valid_perplexity)
				             + '\n')
