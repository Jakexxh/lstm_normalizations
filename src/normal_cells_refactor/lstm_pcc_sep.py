...
class PCCLSTMCell(RNNCell):
    ...
    def _line_sep(self,
        ...
        with vs.variable_scope(scope) as outer_scope:
            ...
            pcc_xh = self.pcc_norm(x, W_xh, 'cn_xh')

            pcc_ih = self.pcc_norm(h, W_ih, 'cn_ih') + pcc_xh[:, :h_size]
            pcc_jh = self.pcc_norm(h, W_jh, 'cn_jh') + pcc_xh[:, h_size:h_size * 2]
            pcc_fh = self.pcc_norm(h, W_fh, 'cn_fh') + pcc_xh[:, h_size * 2:h_size * 3]
            pcc_oh = self.pcc_norm(h, W_oh, 'cn_oh') + pcc_xh[:, h_size * 3:]
